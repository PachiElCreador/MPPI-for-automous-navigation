function [map, cost_map] = LaberintoS_2()
    ...
    %%----------------------------------------------------------------------
    border_thickness = 1;  % Thickness of the borders
    ...
    %%----------------------------------------------------------------------
    % 2. Corners (3x3 meters) with cost = 10000
    %%----------------------------------------------------------------------
    corner_size = 3;
    ...
    %%----------------------------------------------------------------------
    % 2bis. Vertical lines (5 meters) on the left and right borders
    %       with cost = 1000
    % Left: every 20 meters starting at y=10 (10, 30, 50, 70, 90)
    % Right: every 20 meters starting at y=20 (20, 40, 60, 80, 100)
    %%----------------------------------------------------------------------

    % Left border (x=1), centers at y=10, 30, 50, 70, 90
    left_centers = 10:10:90;
    ...
    % Right border (x=100), centers at y=20, 40, 60, 80, 100
    right_centers = 20:10:100;
    ...
    %%----------------------------------------------------------------------
    % 3. Horizontal lines (85 meters in X)
    ...
    line_y_start_0 = [10, 20, 50, 60, 90];
    for y_val = line_y_start_0
        % Segment 10000 (first 5 meters: x=0..4)
        for x_val = 0 : 4
        ...
        % Segment 10000 (last 5 meters: x=80..84)
        for x_val = 75 : 79
    ...
    line_y_start_15 = [30, 40, 70, 80];
    for y_val = line_y_start_15
        % Segment 10000 (first 5 meters: x=15..19)
        for x_val = 17 : 19
            cost_map(x_val+1, y_val) = lineEnds;
            setOccupancy(map, [x_val+1, y_val], 1);
        end
        ...
        % Segment 10000 (last 5 meters: x=95..99)
        for x_val = 95 : 99
    ...
    %%----------------------------------------------------------------------
    % 4. Random obstacles (1-cell points) with cost = 10000
    %%----------------------------------------------------------------------
    num_obstacles = 1;  
    ...
    for i = 1:num_obstacles
        while true
            ...
            % Check if it overlaps with a previous obstacle
            overlap = any(obstacles(:,1) == x_rand & obstacles(:,2) == y_rand);

            % Check if it falls into occupied areas (borders, corners, lines)
